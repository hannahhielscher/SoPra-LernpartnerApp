...
class Vorschlag(bo.BusinessObject):
    ...
    def get_all(self):
        inhalt = [self.id, self._main_person_id, self._match_quote, self._lernfaecher_id, self._profil_id]
        return inhalt

    def __str__(self):
        """ Umwandlung der Attributwerte des Objekts in einen String"""
        return "Profil: {}, {}, {}".format(self._main_person_id, self._match_quote, self._lernfaecher_id, self._profil_id)
